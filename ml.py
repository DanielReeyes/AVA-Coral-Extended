...
import pandas as pd
import numpy as np
import classificador_ml as preditor
...
def sumarizar(i, disciplina, classificador, df, result, coral=False):
    ...
    result.loc[i,'Disciplina'] = disciplina
    ...
    if (coral==True):
        result.loc[i, classificador + 'Coral']                      = df['Acur'].mean()
        result.loc[i, classificador + 'CoralDP']                    = df['Acur'].std(ddof=1)
        result.loc[i, 'GMeanCoral']                                 = gmean
        result.loc[i, classificador + 'PrecisionSucessoCoral']      = df['Precision Sucesso'].mean()
        result.loc[i, classificador + 'PrecisionInsucessoCoral']    = df['Precision Insucesso'].mean()
        result.loc[i, classificador + 'RecallSucessoCoral']         = df['Recall Sucesso'].mean()
        result.loc[i, classificador + 'RecallInsucessoCoral']       = df['Recall Insucesso'].mean()
    else:
        result.loc[i, classificador]                        = df['Acur'].mean()
        result.loc[i, classificador + 'DP']                 = df['Acur'].std(ddof=1)
        result.loc[i, 'GMean']                              = gmean
        result.loc[i, classificador + 'PrecisionSucesso']   = df['Precision Sucesso'].mean()
        result.loc[i, classificador + 'PrecisionInsucesso'] = df['Precision Insucesso'].mean()
        result.loc[i, classificador + 'RecallSucesso']      = df['Recall Sucesso'].mean()
        result.loc[i, classificador + 'RecallInsucesso']    = df['Recall Insucesso'].mean()      
...
#Carrega dataset
df = pd.read_csv('Dataset/dataset_m3_m6.csv', sep=';')
