...
def process(df, disciplina_s, modulo_s, classificador, use_coral):
    ...
    for name, fold in df_s_folds:
        ...
        result.loc[i,'Classificador']           = classificadores[classificador]
        result.loc[i,'Disciplina']              = disciplina_string
        result.loc[i,'Turma']                   = 'T' + str(i)
        result.loc[i,'Coral']                   = use_coral
        result.loc[i,'Acur']                    = accuracy * 100
        result.loc[i,'Precision Sucesso']       = (TN / (TN + FN)) * 100 
        result.loc[i,'Recall Sucesso']          = RecallSucesso * 100 
        result.loc[i,'Precision Insucesso']     = (TP / (TP + FP)) * 100 
        result.loc[i,'Recall Insucesso']        = RecallInsucesso * 100 
        result.loc[i,'TreinoSucesso']           = len(target_s[target_s == 0]) / len(target_s) * 100
        result.loc[i,'TreinoInsucesso']         = len(target_s[target_s == 1]) / len(target_s) * 100
        result.loc[i,'TreinoDesbalanceamento']  = (len(target_s[target_s == 1]) / len(target_s) * 100) / (len(target_s[target_s == 0]) / len(target_s) * 100) * 5
        result.loc[i,'TesteSucesso']            = len(target_t[target_t == 0]) / len(target_t) * 100
        result.loc[i,'TesteInsucesso']          = len(target_t[target_t == 1]) / len(target_t) * 100
        result.loc[i,'TesteDesbalanceamento']   = (len(target_t[target_t == 1]) / len(target_t) * 100) / (len(target_t[target_t == 0]) / len(target_t) * 100) * 5
        result.loc[i,'GMean']                   = gmean
        result.loc[i,'TPR']                     = RecallInsucesso
        result.loc[i,'TNR']                     = RecallSucesso
